    ...
    <div>
        <div v-for="(item, index) in menuListData" :key="index">
            <el-submenu v-if="item.children.length > 0" :key="index" :index="item.id">
                ...
                <menuItem :menuListData="item.children">
                </menuItem>
            </el-submenu>
            <el-menu-item v-else :key="index + 1" :index="item.id" @click="menuItemSelected(item)">
...
export default {
    ...
    methods: {
        menuItemSelected(menu) {
            if (this.mainTabs.find(item => item.id == menu.id)) {
